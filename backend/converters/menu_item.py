...
def menu_item_dbo_to_dto(dbo: MenuItemDBO) -> MenuItemDTO:
    dto = MenuItemDTO(id=dbo.id, name=dbo.name, description=dbo.description, price=dbo.price, size=dbo.size, image_url=dbo.image_url, active=dbo.active, is_taxable=dbo.is_taxable, tax_rate=dbo.tax_rate) # Only take DTO attribute that has init=True
